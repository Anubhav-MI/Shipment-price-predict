...
class ModelTrainer:
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        ...
        try:
            ...
            # Read base model score, default to very low for testing
            model_config = self.model_trainer_config.UTILS.read_yaml_file(MODEL_CONFIG_FILE)
            base_model_score = float(model_config.get("base_model_score", -99999))

            # Always save the model for testing purposes
            cost_model = CostModel(preprocessing_obj, best_model)
            trained_model_path = self.model_trainer_config.trained_model_file_path
            self.model_trainer_config.UTILS.save_object(trained_model_path, cost_model)
            logging.info(f"Saved best model regardless of score. Score: {best_model_score}")
